...
public class prova2 {
	...
	public static void main(String[] args) {
		...
		String[] valori = {"6-8 s svsssszslpsp",
				"3-4 n gncn",
		...
		{
			String[] spazio = valori[i].split(" ");
			String[] numeri = spazio[0].split("-");
			String ricerca = spazio[1];
			String parola = spazio[2];
			int volte = 0;
			int indice1 = 0;
			int indice2 = 0;
			System.out.println("Valore; " + valori[i]);
			/*if((parola.indexOf(ricerca.charAt(0))+1) == Integer.valueOf(numeri[0]))
			{	
				System.out.println("Indice: " + parola.indexOf(ricerca.charAt(0)));
				System.out.println("Primo indice; " + Integer.valueOf(numeri[0]));
				indice1++;
			}	
			if((parola.lastIndexOf(ricerca.charAt(0))+1) == Integer.valueOf(numeri[1]))
			{	
				System.out.println("Secondo indice; " + Integer.valueOf(numeri[1]));
				indice2++;
			}	
			if((indice1+indice2) == 1)
				ok++;*/
			for(int j = 0; j < parola.length(); j++){
				char[] n = parola.toCharArray();
				if (String.valueOf(n[j]).equals(ricerca)){
					if ((j+1)==Integer.valueOf(numeri[0]))
						volte++;
					if ((j+1)==Integer.valueOf(numeri[1]))
						volte++;
				}
			}
			if (volte != 1)
				System.out.println("Errore; " + valori[i]);
			else
				ok++;
			/*if (volte < Integer.valueOf(numeri[0]) || volte > Integer.valueOf(numeri[1]))
			{
				System.out.println("Errore; " + valori[i]);
			}
			else 
		...
		System.out.println("Numero ok: "+ok);
	}
