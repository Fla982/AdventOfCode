				...
				for (int g = 0; g < array2.length; g++){
					...
					for( int h = 0; h < array.length; h++){
						...
						if (array[h].trim().endsWith("red")){
							if (Integer.valueOf(value[0].trim())> red) red = Integer.valueOf(value[0].trim());
						}
						else if (array[h].trim().endsWith("green")){
							if (Integer.valueOf(value[0].trim())> green) green = Integer.valueOf(value[0].trim());
							
						}
						else if (array[h].trim().endsWith("blue")){
							if (Integer.valueOf(value[0].trim())> blue) blue = Integer.valueOf(value[0].trim());
							
						}
					}
				}
				int prodotto = red * green *blue;
				somma += prodotto;
				/*if (impossible == false) {
